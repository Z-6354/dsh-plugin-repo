...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Ghost button: transparent surface, thin border, label text — matches DSH's
    // secondary/ghost action look without the flat panel background.
    const btnStyle = {
    ...
    function PluginRepoSection() {
      ...
      return h('div', { style: { fontFamily: T.fontFamily, fontSize: '14px', color: T.labelPrimary } },
        ...
        h('div', { style: { display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' } },
          h('input', { type: 'text', placeholder: '对方地址：域名走 https、IP 走 http，可带 :端口（如 dsh.example.com 或 192.168.1.10:8443）', value: remoteAddr, onChange: (e) => setRemoteAddr(e.target.value), style: { ...inputStyle, width: '360px' } }),
        ...
        repoPkgs.length === 0
          ...
          : h('div', null,
              ...
              repoPkgs.slice(0, shownRepo).map((p) => h('div', { key: p.filename, style: rowStyle },
