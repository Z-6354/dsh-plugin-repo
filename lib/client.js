  ...
  factory: (require) => {
    ...
    function PluginRepoSection() {
      ...
      return h('div', { style: { fontFamily: 'inherit', fontSize: '14px' } },
        ...
        // ---- search roots ----
        h('h3', { style: { marginTop: '20px' } }, '搜索根目录'),
        h('div', { style: { display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap', marginBottom: '6px' } },
          h('input', { type: 'text', placeholder: '输入插件目录路径（如 D:\\0HAN\\Work\\my-plugins）', value: newRoot, onChange: (e) => setNewRoot(e.target.value), style: { padding: '4px 8px', fontSize: '13px', width: '320px' } }),
          h('button', { onClick: addRoot, disabled: busy, style: btnStyle }, '添加路径'),
        ),
        roots.length === 0
          ? h('p', { style: small }, '（仅默认位置 ~/.dsh）')
          : h('div', null, roots.map((r) => h('div', { key: r, style: { padding: '2px 0', fontSize: '12px', color: 'rgba(128,128,128,0.85)', wordBreak: 'break-all' } }, r))),
