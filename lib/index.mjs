...
function apply(ctx, config = {}) {
      ...
      if (root.endsWith('node_modules')) {
        ...
        for (const entry of entries) {
          ...
          try {
            ...
            const pkg = JSON.parse(readFileSync(pj, 'utf8'))
            if (!pkg.name || isOfficial(pkg.name) || !isBundle(pkg)) continue
  ...
  //   - a bare domain/IP ("192.168.1.10", "dsh.example.com")
  //   - a domain/IP + :port ("192.168.1.10:3080", "dsh.example.com:8443")
  //   - a full URL ("https://dsh.example.com", "http://1.2.3.4:9000")
  // Default scheme: https for hostnames (public faces are usually TLS-terminated
  // behind a reverse proxy on 443), http for bare IPv4 addresses (LAN peers on
  // the DSH web port). Explicit scheme/:port always wins.
  function remoteBase(rawHost, rawPort) {
    let host = String(rawHost || '').trim()
    if (!host) throw new Error('远程地址不能为空')
    let port = rawPort ? String(rawPort) : ''
    let scheme = ''
    // Split off an explicit scheme.
    const schemeMatch = /^(https?):\/\/(.+)$/i.exec(host)
    if (schemeMatch) { scheme = schemeMatch[1].toLowerCase(); host = schemeMatch[2] }
    // Split off a trailing :port from the host portion.
    const hostPortMatch = /^(.+):(\d+)$/.exec(host)
    if (hostPortMatch) { host = hostPortMatch[1]; port = hostPortMatch[2] }
    // Strip an accidental path/prefix.
    host = host.replace(/\/.*$/, '')
    // A bare IPv4 (dotted quad) is a LAN peer → http on the DSH port; anything
    // else (hostname, domain) is a public face → https on 443 unless overridden.
    const isIPv4 = /^\d{1,3}(\.\d{1,3}){3}$/.test(host)
    if (!scheme) scheme = isIPv4 ? 'http' : 'https'
    if (!port) port = isIPv4 ? '3080' : '443'
  ...
  function doInstall(filename) {
    ...
    // Link the packed copy under ~/.dsh/plugins so pnpm resolves it locally on
    // any OS (the previous Windows-style file:C:// URL broke on Linux).
    deps[plan.name] = `link:${dest}`
  ...
  async function dispatch(req, res) {
    ...
    try {
      ...
      if (rest === '/api/install-all' && method === 'POST') {
        ...
        const installed = []
        for (const filename of files) {
          try { installed.push({ ok: true, ...doInstall(filename) }) }
          catch (err) { installed.push({ ok: false, filename, error: String((err && err.message) || err) }) }
        }
        json(res, 200, { ok: true, installed }); return
      }
