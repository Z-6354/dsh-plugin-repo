...
function apply(ctx, config = {}) {
  ...
  async function pushTgzToRemote(rawHost, rawPort, filename) {
    filename = basename(filename)
    ...
    const parsed = parseVersionFromFilename(filename)
    const base = remoteBase(rawHost, rawPort)
    const qs = parsed.name ? `?name=${encodeURIComponent(parsed.name)}` : ''
    ...
    return {
      filename,
      name: (data && data.name) || parsed.name || filename,
      version: (data && data.version) || parsed.version || '',
  ...
  async function dispatch(req, res) {
    ...
    try {
      ...
      if (rest === '/api/remote-push' && method === 'POST') {
        ...
        for (const filename of files) {
          try {
            pushed.push({ ok: true, ...await pushTgzToRemote(host, port, filename) })
          } catch (err) {
            pushed.push({ ok: false, filename, error: String((err && err.message) || err) })
          }
        }
        json(res, 200, { ok: true, remoteHost: host, pushed }); return
      }
  ...
  ctx.effect(() => tools.register(defineTool({
    ...
    async execute(args) {
      ...
      for (const filename of files) {
        try {
          pushed.push({ ok: true, ...await pushTgzToRemote(args.host, args.port || '', filename) })
        } catch (e) { pushed.push({ ok: false, filename, error: String((e && e.message) || e) }) }
      }
