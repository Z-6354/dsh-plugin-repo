...
function apply(ctx, config = {}) {
  ...
  // ---------- installed non-official plugin detection ----------
  ...
  async function dispatch(req, res) {
    ...
    try {
      ...
      if (rest === '/api/installed' && method === 'GET') {
        json(res, 200, { ok: true, installed: scanInstalledNonOfficial() }); return
      }

      if (rest === '/api/unofficial' && method === 'GET') {
        json(res, 200, { ok: true, installed: scanInstalledNonOfficial(), repo: listRepoPackages() }); return
      }
  ...
  ctx.effect(() => tools.register(defineTool({
    ...
    parameters: {},
    output: { schema: { type: 'object', additionalProperties: false, properties: { ok: { type: 'boolean', required: true }, installed: { type: 'array', required: true, items: { type: 'object', additionalProperties: false, properties: { name: { type: 'string', required: true }, version: { type: 'string' } } } }, repo: { type: 'array', required: true, items: { type: 'object', additionalProperties: false, properties: { name: { type: 'string', required: true }, version: { type: 'string' }, filename: { type: 'string', required: true }, size: { type: 'integer' } } } }, error: { type: 'string' } } }, render: (_a, v) => [{ type: 'text', text: JSON.stringify(v) }] },
